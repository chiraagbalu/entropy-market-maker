...
async function downloadDatasets(downloadDatasetsOptions) {
    ...
    for (const symbol of datasetsSymbols) {
        for (const dataType of dataTypes) {
            ...
            (0, debug_1.debug)('dataset download started for %s %s %s from %s to %s', exchange, dataType, symbol, from, to);
            if (daysCountToFetch > 1) {
                ...
                await downloadDataSet(getDownloadOptions({
                    exchange,
                    symbol,
                    apiKey,
                    downloadDir,
                    ...
                    format,
                    getFilename,
                    date: (0, handy_1.addDays)(startDate, daysCountToFetch - 1)
                }), skipIfExists);
            }
            ...
            await (0, p_map_1.default)((0, handy_1.sequence)(daysCountToFetch - 1, 1), // this will produce Iterable sequence from 1 to daysCountToFetch - 1 (as we already downloaded data for the first and last day)
            (offset) => downloadDataSet(getDownloadOptions({
                exchange,
                symbol,
                apiKey,
                downloadDir,
                ...
                format,
                getFilename,
                date: (0, handy_1.addDays)(startDate, offset)
            }), skipIfExists), { concurrency: CONCURRENCY_LIMIT });
            const elapsedSeconds = (new Date().valueOf() - startTimestamp) / 1000;
            (0, debug_1.debug)('dataset download finished for %s %s %s from %s to %s, time: %s seconds', exchange, dataType, symbol, from, to, elapsedSeconds);
        }
    }
}
exports.downloadDatasets = downloadDatasets;
async function downloadDataSet(downloadOptions, skipIfExists) {
    if (skipIfExists && (0, fs_extra_1.existsSync)(downloadOptions.downloadPath)) {
        (0, debug_1.debug)('dataset %s already exists, skipping download', downloadOptions.downloadPath);
    }
    else {
        return await (0, handy_1.download)(downloadOptions);
    }
}
function getDownloadOptions({ apiKey, exchange, dataType, date, symbol, format, downloadDir, getFilename }) {
    const year = date.getUTCFullYear();
    const month = (0, handy_1.doubleDigit)(date.getUTCMonth() + 1);
    const day = (0, handy_1.doubleDigit)(date.getUTCDate());
