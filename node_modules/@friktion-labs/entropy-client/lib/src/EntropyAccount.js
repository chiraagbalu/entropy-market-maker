...
class EntropyAccount {
    ...
    toPrettyString(groupConfig, entropyGroup, cache) {
        ...
        for (let i = 0; i < this.perpAccounts.length; i++) {
            ...
            const normFactor = fixednum_1.I80F48.fromNumber(Math.pow(10, entropyGroup.tokens[i].decimals -
                entropyGroup.tokens[layout_1.QUOTE_INDEX].decimals));
            const perpAccount = this.perpAccounts[i];
            const perpMarketInfo = entropyGroup.perpMarkets[i];
            lines.push(`${market.name}: ${this.getBasePositionUiWithGroup(i, entropyGroup).toFixed(4)} / ${market.name}: ${fixednum_1.I80F48.fromNumber(this.getBasePositionUiWithGroup(i, entropyGroup)).mul(normFactor).mul(cache.priceCache[i].price).toFixed(4)} / ${(perpAccount.getQuotePosition(cache.perpMarketCache[i]).toNumber() /
