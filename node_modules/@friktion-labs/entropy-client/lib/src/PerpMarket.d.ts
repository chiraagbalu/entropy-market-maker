...
import { BookSide, FillEvent, EntropyAccount, EntropyCache, MetaData, PerpEventQueue, PerpMarketConfig } from '.';
...
export default class PerpMarket {
     ...
     */
    getCurrentFundingRate(group: EntropyGroup, cache: EntropyCache, marketIndex: number, bids: BookSide, asks: BookSide): number;
    ...
    loadOrdersForAccount(connection: Connection, account: EntropyAccount): Promise<import("./book").PerpOrder[]>;
    uiToNativePriceQuantity(price: number, quantity: number): [BN, BN];
    toPrettyString(group: EntropyGroup, perpMarketConfig: PerpMarketConfig): string;
}
