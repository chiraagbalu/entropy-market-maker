...
import {
  ...
  function makeMarketUpdateInstructions(
    ...
    const takerSize = marketContext.params.takePerc;
    ...
    //if ftx pricing > best offer (ftx: 30k, BO: 29k)
    //lift best offer
    //if ftx pricing < best bid (ftx: 30k, BB: 31k)
    //hit best bid
    //however we also need to make sure its above some sort of threshold
  
    const takeSize = (equity * takerSize) / fairValue;
